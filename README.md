...
1. **制作模板** 📝：导入任意 `.docx`，在原格式预览里点光标 / 选文字，插入 `{@field}`（文字）或 `{%field}`（图片）占位符，保存为模板。
2. **修改模板** 🔧：导入做好的模板，重命名 / 删除字段、改描述和默认格式 —— 重命名会自动同步文档里所有占位符。
3. **填写模板** ✏️：导入模板，应用自动识别字段并生成表单，每个文字字段可独立选字体 / 字号 / 颜色，图片字段直接上传 PNG/JPG，最后导出填好的 `.docx`。
...
|  | 段落 textarea 自动高度（同时支持手动拖拽和滚动） |
|  | 字段统一管理：名称、描述、默认字体 / 字号 / 颜色 |
...
| 填写模板 | 自动识别字段并生成表单 |
|  | 每处独立选字体 / 字号 / 颜色（系统字体全量，~200 个常见 Windows 装机量） |
...
4. **插入字段**有两种方式：
   - **预览里**：点光标定位，或者选中要替换的一段文字，点工具栏 `+ 文字字段` / `+ 图片字段`，弹窗里填名称 / 描述 / 默认格式 → 占位符插到那个位置（选中的文字会被覆盖）
   - **卡片里**：直接在 textarea 里改文本，手写 `{@name}` 也行，应用会自动登记字段元数据
5. 卡片头部的"字段汇总"列出所有字段。点字段名可以再编辑描述、默认格式
6. 右侧段落卡片的 textarea 会自动随内容增高（长文本不用手动拉才看得全），也保留手动拖拽与滚动
7. 改完点 `刷新预览` 看效果，最后 `保存为模板`
...
字段卡片的初始字体 / 字号 / 颜色，来自模板里存的"默认格式"（`template/fields.json`）。第一次填的时候不用每个字段重新选。
...
**字段元数据**（描述、默认格式）保存在 `.docx` 包内的 `template/fields.json`：
...
兼容 v1：早期模板没有 `default*` 字段，应用会回退到"宋体 / 小四 / 黑色"。
...
选了 JSON 方案，关键是 `buildTemplate` 写入 fields.json 时同步加 content type 声明：
