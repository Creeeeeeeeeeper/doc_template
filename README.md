...
1. **制作模板** 📝：导入任意 `.docx`，在原格式预览里点光标 / 选文字，插入 `{@field}`（文字）或 `{%field}`（图片）占位符，保存为模板。占位符在编辑区渲染为高亮块，支持双击编辑、右键删除、Backspace 整块删除。可同时导出 Excel 空白表供他人批量填写。
2. **修改模板** 🔧：导入做好的模板，重命名 / 删除字段、改描述 —— 重命名会自动同步文档里所有占位符。双击图片占位符可修改名称和图片尺寸配置（自适应模式、等比缩放、最大最小宽高，单位为厘米，可保留两位小数）。
3. **填写模板** ✏️：导入模板，应用自动识别字段并生成表单。同一字段名出现在多处时，各处独立保存字体/字号/颜色，文本内容联动同步。图片字段可配置尺寸（宽度/高度/等比自适应，单位为厘米），上传后可直接本地预览，填写时自动按实际图片尺寸和配置计算最终大小，最后导出填好的 `.docx`。支持导入填好的 Excel 批量生成 Word 文件。
...
|  | 右键占位符弹出"删除占位符"菜单 |
|  | 双击占位符弹出编辑框，可修改名称、描述和该位置的字体 / 字号 / 颜色（文字）或图片尺寸配置（图片） |
...
|  | 📦 导出模板+Excel：同时保存 Word 模板和 Excel 空白填写表 |
| 修改模板 | 字段重命名 —— 同步替换文档里所有 `{@old}`→`{@new}` |
|  | 字段删除 —— 一键抹掉所有占位符 + 元数据 |
|  | 字段汇总区显示出现次数（`name ×3`）和未使用元数据 |
|  | 双击占位符可修改名称（全局同步重命名）、描述和该位置样式 |
...
|  | 图片字段支持 PNG / JPEG / GIF |
|  | 图片尺寸可配置：宽度/高度/等比自适应、保持宽高比、最大最小宽高约束（单位：cm，保留两位小数） |
|  | 图片尺寸配置持久化到 fields.json，模板保存后下次打开仍有效 |
|  | 上传图片后显示本地缩略预览（自适应显示，带边框和阴影） |
|  | 导出时自动读取实际图片尺寸，按配置计算最终大小（不再硬编码 240×240） |
|  | 📥 批量导入填写：导入填好的 Excel，自定义文件名模板，批量生成 Word |
|  | 文件名模板支持 `{n}` 自动编号 + `{字段名}` 插入字段值 |
|  | 批量导出时检测图片字段并警告无法批量填写 |
|  | 文件名非法字符校验（`\/:*?"<>|`） |
...
9. 点 `📦 导出模板+Excel` 可同时保存 Word 模板和 Excel 空白填写表（表头为字段名，留 10 行空白供填写），Excel 可发布到腾讯文档等平台让他人填写
...
5. **图片字段**：选 PNG/JPEG/GIF 图片
   - 上传后立即显示本地预览缩略图，方便检查是否选对
   - 尺寸配置单位为 **厘米（cm）**，支持两位小数，例如 `2.00`
6. `生成并保存 DOCX` 选输出路径
7. **批量导入**：点 `📥 批量导入填写`，选择填好的 Excel 文件，配置文件名模板（支持 `{n}` 自动编号、`{字段名}` 插入字段值），选择输出文件夹，自动生成所有 Word 文件
...
`fitMode` 取值：`"width"`（宽度自适应）、`"height"`（高度自适应）、`"contain"`（在最大约束内等比缩放）。`maintainRatio` 为 `true` 时保持图片原始宽高比。`maxWidth` / `maxHeight` / `minWidth` / `minHeight` 的单位是 **厘米（cm）**，支持两位小数。

兼容旧模板：如果早期模板里保存的是像素值（例如 300 / 400 / 50），应用加载时会自动识别并换算成厘米。
...
|  | @tauri-apps/plugin-fs ^2.0.0 | 写文件到任意路径（批量导出） |
|  | xlsx (SheetJS) | Excel 读写（批量填写） |
...
3. 先把厘米换算成像素（按 `96dpi / 2.54`），再做尺寸计算
4. 最终尺寸 clamp 到 `[minWidth..maxWidth] × [minHeight..maxHeight]`
...
- ✅ 双击占位符编辑（名称、描述 + per-occurrence 样式 / 图片尺寸配置）
- ✅ 鼠标悬停占位符 0.5s 显示详情 tooltip
- ✅ per-occurrence 样式：同一字段名在不同位置可有不同字体 / 字号 / 颜色
- ✅ 图片占位符尺寸配置：宽度/高度/等比自适应、保持宽高比、最大最小宽高
- ✅ 图片尺寸配置持久化到 fields.json（per-field 级别，v4 格式，单位 cm）
- ✅ 导出时自动读取实际图片尺寸，按 imageConfig 动态计算最终大小
- ✅ 兼容旧模板中的 px 图片尺寸配置，加载时自动换算为 cm
- ✅ 填写模式图片上传后本地预览（缩略图 + 边框 + 阴影）
...
- ✅ 字体选择弹窗不再自动展开（改为点击触发）
- ✅ 批量导出：制作模板时同时导出 Word 模板 + Excel 空白填写表
- ✅ 批量导入：导入填好的 Excel，自定义文件名模板，批量生成 Word 到指定文件夹
- ✅ 文件名模板：`{n}` 自动编号 + `{字段名}` 插入值 + 实时预览
- ✅ 批量导出时图片字段警告（Excel 无法批量填写图片）
- ✅ 文件名非法字符校验
- ✅ 自定义确认对话框（替代被 Tauri 禁用的原生 confirm）
- ✅ 颜色选择器：色块预览 + 色值显示
